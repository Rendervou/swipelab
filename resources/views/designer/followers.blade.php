...
<div class="designers-container">
    ...
    <div class="designers-header">
        <div class="header-title">{{ request()->routeIs('designer.followers') ? 'Followers of ' : 'Following ' }} {{ $user->name }}</div>
        <div class="header-subtitle">
            {{ $designers->total() }} {{ request()->routeIs('designer.followers') ? 'follower' : 'designer' }}{{ $designers->total() !== 1 ? 's' : '' }}
        </div>
    </div>
    ...
    @else
        <div style="text-align: center; padding: 3rem; background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1); max-width: 1200px; margin: 0 auto;">
            <p style="color: #a0aec0; font-size: 1.1rem;">No {{ request()->routeIs('designer.followers') ? 'followers' : 'designs' }} yet</p>
        </div>
    @endif
</div>
@endsection
